...
class BigramMarkov(object):
    ...
    def train(self, filepath):
        def text_to_knowledge(text):
            ...
            tokens = tokenize_text(text)
            text_knowledge = defaultdict(list)
            for w1, w2 in yield_bigrams():
                text_knowledge[w1].append(w2)
            return text_knowledge
    ...
    def generate_phrase(self, min_chars=100, max_chars=140):
        ...
        w1, w2 = random_bigram_key, random.choice(self.knowledge[random_bigram_key])
        phrase = '  '

        while phrase[-2] not in '.!?' and len(phrase) < max_chars:
            phrase += '%s ' %(w1)
            w1, w2 = w2, random.choice(self.knowledge[w2])
...
def main():
    mk = TrigramMarkov("corpuses/birds-rights-activist-corpus.txt")
    mk.train("corpuses/tswift-corpus.txt")
    mk.train("corpuses/cocoabean-corpus.txt")
